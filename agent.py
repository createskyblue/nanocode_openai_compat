...
class ToolAgent:
    ...
    def _build_system_prompt(self):
        ...
        # 基础指令
        base_instructions = """你是一个有用的AI助手，可以使用工具帮助用户，选择合适工具。如果需要多个步骤，请逐步执行。

## 主动记录（可选）
当用户表达重要的个人信息、偏好、习惯、项目信息、任务进度、决定结论时，可以将信息记录到MEMORY.md文件。
记录格式：简洁中文，不超过50字，不使用表情符号。
例如："用户偏好使用tab缩进，代码风格简洁优先。"
注意：记录是辅助功能，优先正常回答用户问题。"""
